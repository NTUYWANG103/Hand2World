...
class T5Attention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, pos_bias=None):
        """``x`` [B, L1, C], ``context`` [B, L2, C] (default = ``x``), ``mask`` [B, L2] or
        [B, L1, L2]. T5 attention has NO 1/sqrt(d) scaling."""
...
class T5FeedForward(nn.Module):
    def __init__(self, dim, dim_ffn, dropout=0.1):
        super().__init__()
        self.dim, self.dim_ffn = dim, dim_ffn
...
class T5RelativeEmbedding(nn.Module):
    ...
    def forward(self, lq, lk):
        ...
        # Meta-device path uses no `device=` kwarg (arange not implemented on meta).
        kw = {} if torch.device(type="meta") == device else {"device": device}
        rel_pos = torch.arange(lk, **kw).unsqueeze(0) - torch.arange(lq, **kw).unsqueeze(1)
...
class WanT5EncoderModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    def __init__(self, vocab, dim, dim_attn, dim_ffn, num_heads, num_layers, num_buckets,
                 shared_pos=True, dropout=0.1):
        super().__init__()
        self.shared_pos = shared_pos
    ...
    def from_pretrained(cls, pretrained_model_path, additional_kwargs={}, low_cpu_mem_usage=False, torch_dtype=torch.bfloat16):
        """Load weights from a single safetensors / .pth (no shard split)."""
        import inspect
        ...
        if low_cpu_mem_usage:
            try:
                ...
                load_model_dict_into_meta(
                    model, state_dict, dtype=torch_dtype,
