...
# Copied from VideoX-Fun/videox_fun/models/wan_vae3_8.py
# AutoencoderKLWan3_8: Wan 2.2 VAE with spatial_compression_ratio=16
...
def _causal_conv_with_cache(layer, x, feat_cache, feat_idx):
    """Apply ``layer(x, prev_cache)`` and snapshot the new ``CACHE_T``-frame tail of
    ``x`` into ``feat_cache[feat_idx[0]]``. ``feat_cache=None`` short-circuits to ``layer(x)``."""
...
def _apply_cached_seq(seq, x, feat_cache, feat_idx):
    """Apply an ``nn.Sequential`` of mixed layers, threading ``feat_cache`` through
    ``CausalConv3d`` (via ``_causal_conv_with_cache``) and ``ResidualBlock``; everything
    else falls back to ``layer(x)``."""
    for layer in seq:
        if isinstance(layer, CausalConv3d):
            x = _causal_conv_with_cache(layer, x, feat_cache, feat_idx)
...
class DupUp3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, first_chunk=False) -> torch.Tensor:
        ...
        x = x.view(x.size(0), self.out_channels, self.factor_t, self.factor_s,
                    self.factor_s, x.size(2), x.size(3), x.size(4))
        x = x.permute(0, 1, 5, 2, 6, 3, 7, 4).contiguous()
        x = x.view(x.size(0), self.out_channels, x.size(2) * self.factor_t,
                    x.size(4) * self.factor_s, x.size(6) * self.factor_s)
        if first_chunk:
            x = x[:, :, self.factor_t - 1:, :, :]
        return x
...
class Encoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        x = _apply_cached_seq(self.middle, x, feat_cache, feat_idx)
        x = _apply_cached_seq(self.head, x, feat_cache, feat_idx)
        return x
...
class AutoencoderKLWan2_2_(nn.Module):
    ...
    def __init__(self, dim=160, dec_dim=256, z_dim=16, dim_mult=[1, 2, 4, 4],
        ...
        self.decoder = Decoder3d(
            ...
            temperal_downsample[::-1], dropout,
        )

    def encode(self, x, scale):
        self.clear_cache()
        # ``scale`` = [mean (Z,), 1/std (Z,)].
        mean, inv_std = [item.to(x.device, x.dtype).view(1, self.z_dim, 1, 1, 1) for item in scale]
        x = patchify(x, patch_size=2)
        iter_ = 1 + (x.shape[2] - 1) // 4
        chunks = []
        for i in range(iter_):
            self._enc_conv_idx = [0]
            sl = slice(None, 1) if i == 0 else slice(1 + 4 * (i - 1), 1 + 4 * i)
            chunks.append(self.encoder(
                x[:, :, sl, :, :], feat_cache=self._enc_feat_map, feat_idx=self._enc_conv_idx,
            ))
    ...
    def decode(self, z, scale):
        self.clear_cache()
        mean, inv_std = [item.to(z.device, z.dtype).view(1, self.z_dim, 1, 1, 1) for item in scale]
        x = self.conv2(z / inv_std + mean)
        chunks = []
        for i in range(z.shape[2]):
            self._conv_idx = [0]
            chunks.append(self.decoder(
                x[:, :, i:i + 1, :, :], feat_cache=self._feat_map,
    ...
    def clear_cache(self):
        n_dec = sum(1 for m in self.decoder.modules() if isinstance(m, CausalConv3d))
        n_enc = sum(1 for m in self.encoder.modules() if isinstance(m, CausalConv3d))
        self._conv_num, self._enc_conv_num = n_dec, n_enc
        self._conv_idx, self._enc_conv_idx = [0], [0]
        self._feat_map = [None] * n_dec
        self._enc_feat_map = [None] * n_enc
...
class AutoencoderKLWan3_8(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.model = AutoencoderKLWan2_2_(
            dim=c_dim, z_dim=latent_channels, dim_mult=dim_mult, num_res_blocks=2,
            attn_scales=[], temperal_downsample=temperal_downsample, dropout=0.0,
    ...
    def from_pretrained(cls, pretrained_model_path, additional_kwargs={}):
        ...
        kwargs = {k: v for k, v in additional_kwargs.items() if k in valid}
        model = cls(**kwargs)
